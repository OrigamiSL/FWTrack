...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets,samples_per_epoch, max_gap,
                 ...
                 train_cls=False, pos_prob=0.5, search_size=None, backbone_stride = None, random_rate = None, random_DMD = None, DMD_search_number = None,
                 batch_size = None):
        """
        args:
        ...
        self.DMD_search_number = DMD_search_number
    ...
    def sample_continued_ids(self, visible, num_ids=1, min_id=None, max_id=None,
                            allow_invisible=False, force_invisible=False):
        """ Samples num_ids frames between min_id and max_id for which target is visible
        ...
        # No visible ids
        if len(valid_ids) == 0 or len(valid_ids)-num_ids <= 0:
            return None
        
        len_valid = len(valid_ids)
        choose_range = [i for i in range(len_valid - num_ids)]

        begin = random.choices(choose_range, k=1)[0]

        seq = valid_ids[begin:begin + num_ids]
    ...
    def __getitem__(self, index):   
        ...
        for i in range(self.batch_size):
            data = self.getitem(use_search_frames, choose_multi, use_DMD)

            template_images.append(data['template_images'])
            template_anno.append(data['template_anno'])
            search_images.append(data['search_images'])
            search_anno.append(data['search_anno'])
            # use_multi_frame.append(data['use_multi_frame'])

            # print('use_multi_frame', data['use_multi_frame'])
            if  use_search_frames != 1:
                crop_image.append(data['crop_image'])
                jitter_anno.append(data['jitter_anno'])
                hw.append(data['hw'])
  
        if  choose_multi == False:
            return_data = TensorDict({'template_images': torch.stack(template_images, dim = 1),
                                'search_images': torch.stack(search_images, dim = 1),
                                'search_anno': torch.stack(search_anno, dim = 1),
                                })
            ...
            return_data = TensorDict({'template_images': torch.stack(template_images, dim = 1),
                                'search_images': torch.stack(search_images, dim = 1),
                                'search_anno': torch.stack(search_anno, dim = 1),
                                'crop_image': torch.stack(crop_image, dim = 1),
                                'jitter_anno': torch.stack(jitter_anno, dim = 1),
                                'hw': torch.stack(hw, dim = 1),
                                })
        
        return return_data, choose_multi

    def getitem(self, use_search_frames, choose_multi, use_DMD):
        """
        returns:
        ...
        while not valid:
            ...
            if is_video_dataset:
                ...
                if self.frame_sample_mode == 'causal':
                    ...
                    while search_frame_ids is None:
                        base_frame_id = self._sample_visible_ids(visible, num_ids=1, min_id=self.num_template_frames - 1,
                                                                max_id=len(visible) - use_search_frames)
                        ...
                        search_frame_ids = self.sample_continued_ids(visible, min_id=template_frame_ids[0] + 1,
                                                                max_id=template_frame_ids[0] + self.max_gap + gap_increase,
                                                                num_ids=use_search_frames)
                ...
                search_frame_ids = [1] * use_search_frames
            ...
            try:
                ...
                search_frames, search_anno, meta_obj_test = dataset.get_frames(seq_id, search_frame_ids, seq_info_dict,get_last_frame)
                # only_get_last_frame

                H, W, _ = template_frames[0].shape
                template_masks = template_anno['mask'] if 'mask' in template_anno else [torch.zeros((H, W))] * self.num_template_frames
                search_masks = search_anno['mask'] if 'mask' in search_anno else [torch.zeros((H, W))] * use_search_frames
                ...
                data = self.processing(data, use_search_frames, choose_multi, use_DMD)
