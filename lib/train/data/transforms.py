...
# from lib.models.fwtrack.torch_modwt_2d_batch import modwt_2d, get_ker, get_modwt2d, get_modwt_mra, 
from lib.models.fwtrack.modwt_mra import modwt_2d, get_ker, get_modwt2d, get_modwt_mra, get_all_smooth_kernel
...
class Transform:
    ...
    def __init__(self, *transforms):
        ...
        self._valid_inputs = ['image', 'coords', 'bbox', 'mask', 'att', 'multi_img']
    ...
    def _split_inputs(self, inputs):
        ...
        # print('len_s', len(split_inputs))
        # print('split_inputs', split_inputs)
...
class TransformBase:
    ...
    def __init__(self):
        ...
        self.modwt_kernel = get_ker()
        self.mra_kernel = get_all_smooth_kernel()

        weight1 = torch.FloatTensor(self.modwt_kernel[0]).unsqueeze(0).unsqueeze(0)
        input_weight1 = torch.nn.Parameter(weight1, requires_grad = False)
        weight2 = torch.FloatTensor(self.modwt_kernel[1]).unsqueeze(0).unsqueeze(0)
        input_weight2 = torch.nn.Parameter(weight2, requires_grad = False)
        weight3 = torch.FloatTensor(self.modwt_kernel[2]).unsqueeze(0).unsqueeze(0)
        input_weight3 = torch.nn.Parameter(weight3, requires_grad = False)
        self.weight_list = [input_weight1, input_weight2, input_weight3]
    ...
    def transform_multi_img(self, multi_img, *rand_params):
        """Must be deterministic"""
        return multi_img
...
class ToTensorAndJitter(TransformBase):
    ...
    def transform_multi_img(self, multi_img, brightness_factor):
        # handle numpy array
        multi_img = torch.from_numpy(multi_img.transpose((2, 0, 1)))

        # backward compatibility
        if self.normalize:
            return multi_img.float().mul(brightness_factor/255.0).clamp(0.0, 1.0)
        else:
            return multi_img.float().mul(brightness_factor).clamp(0.0, 255.0)
...
class Normalize_Wavelets(TransformBase):
    """Normalize image"""
    def __init__(self, mean, std, inplace=False, level=3):
        super().__init__()
        # self.mean = mean
        # self.std = std
    ...
    def transform_image(self, image):
        ...
        img_wavelets = get_modwt2d(image, self.modwt_kernel, self.weight_list)
        img_mra = get_modwt_mra(img_wavelets, self.mra_kernel)
        # S
        img_S = img_mra[:, -3:, :, :]
        # D
        img_D = image - img_S

        img_all = torch.cat([img_S, img_D], dim = 1)
        mean_all = torch.cat([self.mean, self.mean], dim = 1)
        std_all = torch.cat([self.std, self.std], dim = 1)
        img_tensor_norm = ((img_all) - mean_all) / std_all  # (1,6,H,W)

        if len(img_tensor_norm.shape) == 4:
            img_tensor_norm = img_tensor_norm.squeeze(0)
...
class RandomHorizontalFlip(TransformBase):
    ...
    def transform_multi_img(self, multi_img, do_flip):
        if do_flip:
            if torch.is_tensor(multi_img):
                return multi_img.flip((2,))
            return np.fliplr(multi_img).copy()
        return multi_img
