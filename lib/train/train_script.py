...
from lib.train.actors import FWTrackActor
# for import modules
import importlib
...
def run(settings):
    ...
    # Create network
    if settings.script_name == "fwtrack":
        net = build_fwtrack(cfg)
    ...
    if settings.script_name == "fwtrack":
        ...
        actor = FWTrackActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg, bins=bins, search_size=search_size)
